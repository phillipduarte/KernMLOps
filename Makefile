...
install-cloudsuite:
	@echo "Installing Cloudsuite web-serving benchmark…"
	@su ${UNAME} -c "bash scripts/setup-benchmarks/install-cloudsuite.sh"

benchmark-cloudsuite:
	@python python/kernmlops collect -v \
 		-c ${KERNMLOPS_CONFIG_FILE} \
 		--benchmark webserving

install-terasort:
	@echo "Installing Hadoop Terasort workload…"
	@bash scripts/setup-benchmarks/install-terasort.sh

benchmark-terasort: install-terasort
	@echo "Collecting data for terasort…"
	@python python/kernmlops collect -v \
	  -c ${KERNMLOPS_CONFIG_FILE} \
	  --benchmark terasort
